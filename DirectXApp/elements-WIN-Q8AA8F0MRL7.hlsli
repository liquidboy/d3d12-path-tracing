...
float random(uint index) {
	...
	float2 seed = float2(rand, rand);
	float2 origin = float2(uint2(index % resolution.x, index / resolution.x)) / float2(resolution);
	return frac(sin(dot(origin + seed, float2(12.9898, 78.233))) * 43758.5453 + rand);
}
