...
void Sample3DSceneRenderer::CreateDeviceDependentResources()
{
	...
	Concurrency::task<void> createTask[10];
	Concurrency::task<void> loadTask[10];

	std::wstring shaders[] = { L"camera.cso", L"begin.cso", L"intersection.cso", L"background.cso", L"diffuse.cso", L"light.cso",L"final.cso", L"render.cso", L"clear.cso" };

	int it;
	for (it = 0;it < _countof(shaders);it++) {
	...
	auto createAssetsTask = (createTask[it-1]).then([this]() {
		...
		for (int i = 0;i < 9;i++) {
			...
			m_cbvHeap[i].Create(d3dDevice, D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV, 4, true);
...
void Sample3DSceneRenderer::Render()
{
	...
	for (int i = 0;i < 8;i++) {
		...
		m_constantBufferData.origin = { 0.0, -2.0, 0.0 };
		m_constantBufferData.normal = { 0.0, 1.0, 0.0 };
		m_constantBufferData.primitiveType = 1;
		m_constantBufferData.primitiveID = 1;
		memcpy(m_mappedConstantBuffer, &m_constantBufferData, sizeof(m_constantBufferData));
		useProgram(2); //Interection shader (diffuse plane)
		
		m_constantBufferData.origin = { 0.0, 0.0, 0.0 };
		m_constantBufferData.radius = 1;
